...
http upgrade的作用是将http协议升级到其他协议。upgrade会产生特定的http header用来确定特定的协议。有两种类型的升级方式，分别是connect和websocket。
...
```
{
  "config_source": "{...}",配置源
  "default_config": "{...}",默认配置
  "apply_default_config_without_warming": "...",是否应用默认配置不警告
  "type_urls": []urls
}
```
...
```
{
  ...
  "ads": "{...}",ads
  "initial_fetch_timeout": "{...}",初始拉取超时时间
  "resource_api_version": "..."资源a
}
```
...
ef-connect-http1.1-terminate.yaml
...
ef-connect-http1.1-tunneling.yaml
...
ef-upgrade-http2-terminate.yaml
...
ef-upgrade-http2-tunneling.yaml
